...
| Tạ Long Khánh (`2A202601197`) | CP1/CP2 Metrics & Dashboard: bổ sung `error_rate_pct` trong [`app/metrics.py`](../app/metrics.py), hoàn thiện [`docs/dashboard-spec.md`](../docs/dashboard-spec.md) và dashboard evidence đủ 6 nhóm chỉ số. | [Integration commit `13bca82`](https://github.com/DucAnhIT03/Day13-b3_HKT/commit/13bca8203d0fa0d431ebdf6c0147f7e4530e2b6f) | Error rate phải dùng cả request thành công và thất bại làm mẫu số; dashboard cần cùng time range, đơn vị và SLO line để so sánh đúng incident. |
| Nguyễn Huy Tỏa (`2A202601697`) | CP2 SRE & Alerts: thiết lập [`config/slo.yaml`](../config/slo.yaml), ba symptom-based rules trong [`config/alert_rules.yaml`](../config/alert_rules.yaml) và runbook Metrics → Traces → Logs tại [`docs/alerts.md`](../docs/alerts.md). | [Integration commit `13bca82`](https://github.com/DucAnhIT03/Day13-b3_HKT/commit/13bca8203d0fa0d431ebdf6c0147f7e4530e2b6f) | Alert theo triệu chứng/SLO phản ánh tác động người dùng và ít nhiễu hơn alert phụ thuộc tên hàm hoặc implementation có thể thay đổi. |
